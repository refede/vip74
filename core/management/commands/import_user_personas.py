...
CSV_PATH = "D:/Users/rflores/Documents/@Refede/Code/db/csv/user_persona.csv"
# -----------------------------------------
    ...
    def handle(self, *args, **options):
        ...
        try:
            with open(csv_file_path, mode="r", encoding="utf-8-sig") as csvfile:
                ...
                for i, row in enumerate(reader):
                    ...
                    id = row.get("id")
                    prefijo = row.get("per_prefijo")
                    nombre = row.get("per_nombre")
                    paterno = row.get("per_apellido_p")
                    materno = row.get("per_apellido_m")
                    nickname = row.get("per_nickname")
                    dni = row.get("per_dni")
                    nacimiento = row.get("per_fecha_nac")
                    formacion = row.get("per_formacion")
                    email = row.get("per_email")
                    telefono = row.get("per_telefono")
                    ingreso = row.get("per_fecha_ing")
                    departamento = row.get("per_departamento")
                    area = row.get("per_area")
                    puesto = row.get("per_puesto")
                    tipo = row.get("per_tipo")
                    # estado          = row.get('per_estado')
                    obs = row.get("per_obs")

                    if not nombre or not paterno:
                        errores_filas.append(
                            f"Línea {line_num}: 'nombre' y 'apellido' son obligatorios."
                        )
                        continue
                    ...
                    modelo_instance = Persona(
                        ...
                        nickname=nickname,
                        dni=dni,
                        nacimiento=self._cast_to_date(
                            nacimiento, "per_fecha_nac", line_num
                        ),
                        formacion=formacion,
                        email=email,
                        telefono=telefono,
                        ingreso=self._cast_to_date(ingreso, "per_fecha_ing", line_num),
        ...
        try:
            with transaction.atomic():
                created_objects = Persona.objects.bulk_create(
                    elementos_a_crear,
