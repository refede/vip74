...
from inspeccion.models import Instructivo, Equipo # CAMBIA 'tu_app'

# --- DEFINE LA RUTA A TU CSV AQUÍ ---
CSV_RELATION_PATH = "D:/Users/rflores/Documents/@Refede/Code/db/csv/inspeccion_instructivo_equipos.csv"
#-----------------------------------------

class Command(BaseCommand):
    help = 'Importa relaciones ManyToMany desde un CSV.'
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.SUCCESS(f"Iniciando importación de relaciones desde: {csv_file_path}"))
        ...
        errores_filas = []
        AmbasThrough = Instructivo.equipos.through # El modelo de la tabla intermedia
        ...
        try:
            with open(csv_file_path, mode='r', encoding='utf-8-sig') as csvfile:
                ...
                for i, row in enumerate(reader):
                    ...
                    instructivo_id_str = row.get('instructivo_id')
                    equipo_id_str = row.get('equipo_id')

                    if not instructivo_id_str or not equipo_id_str:
                        errores_filas.append(f"Línea {line_num}: 'propiedad_id' y 'unidad_id' son obligatorios.")
                        continue
                    
                    try:
                        inst_id = (instructivo_id_str)
                        equi_id = int(equipo_id_str.strip())
                    except ValueError:
                        errores_filas.append(f"Línea {line_num}: IDs ('{instructivo_id_str}', '{equipo_id_str}') deben ser números enteros.")
                        continue
        ...
        try:
            with transaction.atomic():
                ...
                AmbasThrough.objects.bulk_create(
                    m2m_relations_to_create,
